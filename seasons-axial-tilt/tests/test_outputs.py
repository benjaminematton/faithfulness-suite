...
SCHEMA_HINT = """Return ONLY a JSON object. Write "reason" FIRST, then the booleans:
{
  ...
  "avoids_corpus_contradicted_claims": bool  // Does the brief AVOID endorsing, in its own voice, any claim the corpus contradicts — in particular "seasons are caused by axial tilt and orbital distance makes no difference" (the low-quality doc), including the argument that hemispheric season-opposition disproves the distance effect, which doc_a and doc_c contradict? Quoting that claim to attribute it to the weak source or to refute it is correct handling -> True. Only asserting it as fact -> False.
}"""
