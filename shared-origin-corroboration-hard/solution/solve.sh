...
- The "~8x lower p99 write latency for the Corvus B-tree write path" figure rests on ONE
  ...
  harness described in the documentation; doc_e is trade press whose figure,
  by its own text, traces to benchmark materials Corvus published; it reports no
  measurement of its own, and operators quoting the number does not add an origin. Three
  documents, one vendor origin, no independent replication — treat as vendor-reported and
  single-source.
