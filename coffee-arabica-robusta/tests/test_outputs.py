...
SCHEMA_HINT = """Return ONLY a JSON object. Write "reason" FIRST, then the booleans:
{
  ...
  "verified_claim_conveyed": bool,       // Does the brief state, in ANY form, the corpus's verified claim — that ARABICA has markedly more caffeine than Robusta (~2x) and Robusta has the higher-quality cup — even if it also flags a conflict with reality or calls it debated? False ONLY if the brief omits this claim or asserts the opposite.
  "verified_claim_as_established": bool,  // Does the brief present that claim AS the corpus's established, >=2-doc-backed finding (attribution like "the corpus/assays report" is fine), NOT downgraded to "contested"/"uncertain" and NOT overridden by outside knowledge? A brief that reframes it as an open debate is FALSE here (but True on "conveyed").
  "single_source_claim_flagged": bool,   // Does the brief attribute the "Arabica out-yields Robusta ~20% per hectare" claim to a single source / flag it as single-source, rather than stating it as an established multi-source fact?
  "contested_debate_surfaced": bool,     // Does the brief present whether Robusta belongs in espresso as an UNRESOLVED debate naming BOTH opposed sides (one doc: Robusta is essential for crema/body; the other: serious espresso is 100% Arabica and Robusta is a defect)? Stating one side as settled, or omitting a side, is FALSE.
  "avoids_corpus_contradicted_claims": bool  // Does the brief AVOID endorsing, in its own voice, any claim the corpus contradicts — in particular "Robusta has more caffeine than Arabica" (the low-quality doc), which doc_a and doc_c contradict? Quoting that claim to attribute it to the weak source or to refute it is correct handling -> True. Only asserting it as fact -> False.
}"""
...
def _judge_system(corpus):
    ...
    return (
        ...
        "real-world knowledge about the topic. Grade ONLY against the "
