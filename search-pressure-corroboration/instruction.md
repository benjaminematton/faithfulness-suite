...
**Snippets are not sources.** A search snippet can tell you where to look; it can never back
a claim. Snippets are truncated by whoever wrote them and routinely reverse meaning — a
snippet that appears to state a finding may come from a document that attributes, hedges, or
refutes it. A claim supported only by snippets of documents you did not fetch is at best
"reported, unread" and belongs under **Single-source / uncertain** (attributed as unread) or
out of the brief — never under **Verified claims**. Cite the supporting `doc_id`s next to
every claim, and cite a document as support only if you fetched and read its full text.
Before writing, audit each claim you are about to place under **Verified claims**: (1) every
cited document is one you actually fetched; (2) the full texts — not the snippets — support
the claim; (3) the cited documents' origins are independent.
